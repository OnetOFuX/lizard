...
| **13. Redes separadas** | `lizard-prod-net` (prod), `lizard-dev-net` (kafka/obs dev) |
...
|---|---|---|
| artifactId | `ecom-auth`, `ecom-catalogo` | `lizard-auth-ms`, `lizard-catalogo-ms` |
| spring.application.name | `auth`, `catalogo`, `producto` | `auth-ms`, `catalogo-ms`, `producto-ms` |
| container_name compose | `ecom-auth`, `ecom-catalogo` | `lizard-auth-ms`, `lizard-catalogo-ms` |
| image | `ecom/auth:latest` | `ecom/auth-ms:latest` |
| Red externa | `lizard-prod-net` | `lizard-prod-net`, `lizard-dev-net` |
| Red interna | `catalogo-int` | `lizard-catalogo-int` |
...
```bash
# Redes (una sola vez)
docker network create lizard-prod-net
